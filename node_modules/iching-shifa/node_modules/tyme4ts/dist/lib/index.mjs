...
var EventType = /* @__PURE__ */ ((EventType2) => {
  EventType2[EventType2["SOLAR_DAY"] = 0] = "SOLAR_DAY";
  EventType2[EventType2["SOLAR_WEEK"] = 1] = "SOLAR_WEEK";
  EventType2[EventType2["LUNAR_DAY"] = 2] = "LUNAR_DAY";
  EventType2[EventType2["TERM_DAY"] = 3] = "TERM_DAY";
  EventType2[EventType2["TERM_HS"] = 4] = "TERM_HS";
  EventType2[EventType2["TERM_EB"] = 5] = "TERM_EB";
  return EventType2;
})(EventType || {});
...
class YearUnit extends AbstractTyme {
  year;
  constructor(year) {
    super();
    this.year = year;
  ...
  getCompareIndex() {
    return this.year * 1e4;
  }
}
class MonthUnit extends YearUnit {
  month;
...
class DayUnit extends MonthUnit {
  day;
...
class WeekUnit extends MonthUnit {
...
class SecondUnit extends DayUnit {
  hour;
  minute;
  second;
...
class Element extends LoopTyme {
  ...
  constructor(names, indexOrName) {
    super(names === null ? Element.NAMES : names, indexOrName);
  }
  static fromIndex(index) {
    return new Element(null, Element.numeric(index, "element index"));
  }
  static fromName(name) {
...
class Phenology extends LoopTyme {
  ...
  constructor(year, indexOrName) {
    ...
    const y = Phenology.numeric(year, "phenology year");
    if (typeof indexOrName === "number") {
      const size = this.getSize();
...
class NineStar extends LoopTyme {
  ...
  static COLORS = ["\u767D", "\u9ED1", "\u78A7", "\u7EFF", "\u9EC4", "\u767D", "\u8D64", "\u767D", "\u7D2B"];
  constructor(indexOrName) {
  ...
  getColor() {
    return NineStar.COLORS[this.index];
  }
  getElement() {
...
class LunarYear extends YearUnit {
  ...
  getLeapMonth() {
    ...
    for (let i = 0, j = LunarYear.LEAP.length; i < j; i++) {
...
class LunarMonth extends MonthUnit {
  ...
  leap;
  constructor(year, month) {
    const y = LunarMonth.numeric(year, "lunar year");
    const m = LunarMonth.numeric(month, "lunar month");
...
class LunarWeek extends WeekUnit {
  ...
  next(n) {
    ...
    if (n > 0) {
      let weekCount = m.getWeekCount(this.start);
      while (d >= weekCount) {
        d -= weekCount;
        m = m.next(1);
  ...
  getFirstDay() {
    const firstDay = LunarDay.fromYmd(this.year, this.month, 1);
...
class LunarDay extends DayUnit {
  ...
  constructor(year, month, day) {
    const y = LunarDay.numeric(year, "lunar year");
    const m = LunarDay.numeric(month, "lunar month");
...
class SixtyCycleDay extends AbstractTyme {
  ...
  static fromSolarDay(solarDay) {
    ...
    return new SixtyCycleDay(solarDay, month, SixtyCycle.fromIndex(solarDay.subtract(SolarDay.fromYmd(2e3, 1, 7))));
  }
  getSolarDay() {
...
class LunarHour extends SecondUnit {
  ...
  constructor(year, month, day, hour, minute, second) {
    const y = LunarHour.numeric(year, "lunar year");
    const m = LunarHour.numeric(month, "lunar month");
    const d = LunarHour.numeric(day, "lunar day");
    const h = LunarHour.numeric(hour, "hour");
    const n = LunarHour.numeric(minute, "minute");
  ...
  next(n) {
    ...
    const d = this.getLunarDay().next(Math.floor(h / 24));
...
class ShouXingUtil {
  ...
  static gxcSunLon(t) {
    const t2 = t * t;
    return -20.49552 * (1 + (0.016708634 - 42037e-9 * t - 1267e-10 * t2) * Math.cos(-0.043126 + 628.301955 * t - 2732e-9 * t2)) / ShouXingUtil.SECOND_PER_RAD;
  }
  static ev(t) {
  ...
  static qiShuo(isQi, isHigh, jd, pc) {
    const w = isQi ? Math.floor((jd + pc - 2451259) / 365.2422 * 24) * Math.PI / 12 : Math.floor((jd + pc - 2451551) / 29.5306) * ShouXingUtil.PI_2;
    const d = isQi ? isHigh ? ShouXingUtil.qiHigh(w) : ShouXingUtil.qiLow(w) : isHigh ? ShouXingUtil.shuoHigh(w) : ShouXingUtil.shuoLow(w);
    return Math.floor(d + 0.5);
  }
  static calc(isQi, jd, kb, pc, fkb) {
    const size = kb.length;
    let d = 0;
    const j = jd + JulianDay.J2000;
    const f1 = kb[0] - pc;
    const f2 = kb[size - 1] - pc;
    if (j < f1 || j >= 2436935) {
      ...
      const n = fkb.charCodeAt(Math.floor(isQi ? (j - f2) / 365.2422 * 24 : (j - f2) / 29.5306));
      if (n === 49) {
        d += 1;
      } else if (n === 50) {
        d -= 1;
...
class SolarTerm extends LoopTyme {
  ...
  constructor(year, indexOrName) {
    ...
    let y = SolarTerm.numeric(year, "solar term year");
    if (typeof indexOrName === "number") {
...
class SolarHalfYear extends YearUnit {
  ...
  constructor(year, index) {
    const y = SolarHalfYear.numeric(year, "solar year");
...
class SolarSeason extends YearUnit {
  ...
  index;
  constructor(year, index) {
    const y = SolarSeason.numeric(year, "solar year");
...
class SolarMonth extends MonthUnit {
  ...
  constructor(year, month) {
    const y = SolarMonth.numeric(year, "solar year");
...
class SolarWeek extends WeekUnit {
  constructor(year, month, index, start) {
    const y = SolarWeek.numeric(year, "solar year");
    const m = SolarWeek.numeric(month, "solar month");
  ...
  getIndexInYear() {
    ...
    let w = SolarWeek.fromYm(this.year, 1, 0, this.start);
  ...
  getFirstDay() {
    const firstDay = SolarDay.fromYmd(this.year, this.month, 1);
    return firstDay.next(this.index * 7 - this.indexOf(firstDay.getWeek().getIndex() - this.start, 7));
  }
  getDays() {
    const l = [];
...
class SolarDay extends DayUnit {
  ...
  constructor(year, month, day) {
    const y = SolarDay.numeric(year, "solar year");
    const m = SolarDay.numeric(month, "solar month");
  ...
  getConstellation() {
    ...
    const offset = this.day > [19, 18, 20, 19, 20, 21, 22, 22, 22, 23, 22, 21][m] ? 1 : 0;
  ...
  getSolarWeek(start) {
    return SolarWeek.fromYm(this.year, this.month, Math.ceil((this.getDay() + SolarDay.fromYmd(this.year, this.month, 1).getWeek().next(-start).getIndex()) / 7) - 1, start);
  }
  getPhenologyDay() {
  ...
  getNineDay() {
    let start = SolarTerm.fromIndex(this.year + 1, 0).getSolarDay();
  ...
  getLunarDay() {
    let m = LunarMonth.fromYm(this.year, this.month);
  ...
  getNineStar() {
    const winterSolstice = SolarTerm.fromIndex(this.year, 0).getSolarDay();
    const summerSolstice = SolarTerm.fromIndex(this.year, 12).getSolarDay();
    const nextWinterSolstice = SolarTerm.fromIndex(this.year + 1, 0).getSolarDay();
  ...
  getHijriDay() {
    let d = this.subtract(new SolarDay(622, 7, 16));
    const z = Math.floor(d / 10631);
    d -= z * 10631;
    const y = Math.floor((d + 0.5) / 354.366);
    d -= Math.floor(y * 354.366 + 0.5);
    const m = Math.floor((d + 0.11) / 29.51);
...
class SolarTime extends SecondUnit {
  constructor(year, month, day, hour, minute, second) {
    const y = SolarTime.numeric(year, "solar year");
    const m = SolarTime.numeric(month, "solar month");
    const d = SolarTime.numeric(day, "solar day");
    const h = SolarTime.numeric(hour, "hour");
    const n = SolarTime.numeric(minute, "minute");
  ...
  next(n) {
    ...
    const t = this.getSecondsInDay() + n;
    const s = this.indexOf(t, 86400);
    const d = this.getSolarDay().next(Math.floor(t / 86400));
...
class LegalHoliday extends AbstractTyme {
  static NAMES = ["\u5143\u65E6", "\u6625\u8282", "\u6E05\u660E\u8282", "\u52B3\u52A8\u8282", "\u7AEF\u5348\u8282", "\u4E2D\u79CB\u8282", "\u56FD\u5E86\u8282", "\u56FD\u5E86\u4E2D\u79CB", "\u6297\u6218\u80DC\u5229\u65E5"];
...
class AbstractFestival extends AbstractTyme {
  index;
  day;
  event;
  constructor(index, event, day) {
    super();
    this.index = index;
    this.event = event;
    this.day = day;
...
class SolarFestival extends AbstractFestival {
  static NAMES = ["\u5143\u65E6", "\u5987\u5973\u8282", "\u690D\u6811\u8282", "\u52B3\u52A8\u8282", "\u9752\u5E74\u8282", "\u513F\u7AE5\u8282", "\u5EFA\u515A\u8282", "\u5EFA\u519B\u8282", "\u6559\u5E08\u8282", "\u56FD\u5E86\u8282"];
  static DATA = "0VV__0Ux0Xc__0Ux0Xg__0_Q0ZV__0Ux0ZY__0Ux0aV__0Ux0bV__0Uo0cV__0Ug0de__0_V0eV__0Ux";
  constructor(index, event, day) {
    super(index, event, day);
  }
  static fromIndex(year, index) {
    const y = SolarFestival.numeric(year, "solar festival year");
    const i = SolarFestival.numeric(index, "solar festival index");
    if (i < 0 || i >= SolarFestival.NAMES.length) {
      return null;
    }
    const start = i * 8;
    const e = new Event(SolarFestival.NAMES[i], "@" + SolarFestival.DATA.substring(start, start + 8));
    if (y < e.getStartYear()) {
      return null;
    }
  ...
  static fromYmd(year, month, day) {
    const d = SolarDay.fromYmd(year, month, day);
    for (let i = 0, j = SolarFestival.NAMES.length; i < j; i++) {
      const start = i * 8;
      const e = new Event(SolarFestival.NAMES[i], "@" + SolarFestival.DATA.substring(start, start + 8));
...
class LunarFestival extends AbstractFestival {
  ...
  static DATA = "2VV__0002Vj__0002WW__0002XX__0003b___0002ZZ__0002bb__0002bj__0002cj__0002dd__0003s___0002gc__0002hV_U000";
  constructor(index, event, day) {
    super(index, event, day);
  }
  static fromIndex(year, index) {
    const y = LunarFestival.numeric(year, "lunar festival year");
    const i = LunarFestival.numeric(index, "lunar festival index");
    if (i < 0 || i >= LunarFestival.NAMES.length) {
      return null;
    }
    const start = i * 8;
    const e = new Event(LunarFestival.NAMES[i], "@" + LunarFestival.DATA.substring(start, start + 8));
    switch (e.getType()) {
      case 2 /* LUNAR_DAY */:
        const m = e.getMonth(y);
        const d = LunarDay.fromYmd(m[0], m[1], e.getValue(3));
        const offset = e.getValue(5);
  ...
  static fromYmd(year, month, day) {
    const d = LunarDay.fromYmd(year, month, day);
    for (let i = 0, j = LunarFestival.NAMES.length; i < j; i++) {
      const start = i * 8;
      const e = new Event(LunarFestival.NAMES[i], "@" + LunarFestival.DATA.substring(start, start + 8));
      switch (e.getType()) {
        case 2 /* LUNAR_DAY */:
          ...
          if (offset === 0) {
            ...
            const m = e.getMonth(d.getYear());
            const next = d.next(-offset);
          ...
          break;
        case 3 /* TERM_DAY */:
          const term = d.getSolarDay().getTermDay();
  ...
  getSolarTerm() {
    const t = this.getDay().getSolarDay().getTermDay();
...
class RabByungElement extends Element {
  static NAMES = ["\u6728", "\u706B", "\u571F", "\u94C1", "\u6C34"];
  constructor(indexOrName) {
    super(RabByungElement.NAMES, indexOrName);
  }
  static fromIndex(index) {
...
class RabByungYear extends AbstractTyme {
  ...
  constructor(rabByungIndex, elementIndex, zodiacIndex) {
    ...
    const element = RabByungYear.numeric(elementIndex, "element index");
    if (element < 0 || element >= RabByungElement.NAMES.length) {
      throw new Error(`illegal element index: ${element}`);
    }
    const zodiac = RabByungYear.numeric(zodiacIndex, "zodiac index");
...
class RabByungMonth extends MonthUnit {
  ...
  constructor(year, month) {
    RabByungMonth.init();
    const y = RabByungMonth.numeric(year, "rab-byung year");
    const m = RabByungMonth.numeric(month, "rab-byung month");
...
class RabByungDay extends DayUnit {
  ...
  leap;
  constructor(year, month, day) {
    const y = RabByungDay.numeric(year, "rab-byung year");
    const m = RabByungDay.numeric(month, "rab-byung month");
    const d = RabByungDay.numeric(day, "rab-byung day");
...
class HijriYear extends YearUnit {
  constructor(year) {
    const y = HijriYear.numeric(year, "hijri year");
  ...
  isLeap() {
    const i = this.indexOf(this.year - 1, 30);
...
class HijriMonth extends MonthUnit {
  static NAMES = ["\u7A46\u54C8\u5170\u59C6\u6708", "\u8272\u6CD5\u5C14\u6708", "\u8D56\u6BD4\u5C14\xB7\u6556\u5916\u9C81\u6708", "\u8D56\u6BD4\u5C14\xB7\u963F\u8272\u5C3C\u6708", "\u4E3B\u9A6C\u8FBE\xB7\u6556\u5916\u9C81\u6708", "\u4E3B\u9A6C\u8FBE\xB7\u963F\u8272\u5C3C\u6708", "\u8D56\u54F2\u535C\u6708", "\u820D\u5C14\u90A6\u6708", "\u8D56\u4E70\u4E39\u6708", "\u95EA\u74E6\u9C81\u6708", "\u90FD\u5C14\u5580\u5C14\u5FB7\u6708", "\u90FD\u5C14\u9ED1\u54F2\u6708"];
  constructor(year, month) {
    const y = HijriMonth.numeric(year, "hijri year");
    const m = HijriMonth.numeric(month, "hijri month");
...
class HijriDay extends DayUnit {
  static NAMES = ["1\u65E5", "2\u65E5", "3\u65E5", "4\u65E5", "5\u65E5", "6\u65E5", "7\u65E5", "8\u65E5", "9\u65E5", "10\u65E5", "11\u65E5", "12\u65E5", "13\u65E5", "14\u65E5", "15\u65E5", "16\u65E5", "17\u65E5", "18\u65E5", "19\u65E5", "20\u65E5", "21\u65E5", "22\u65E5", "23\u65E5", "24\u65E5", "25\u65E5", "26\u65E5", "27\u65E5", "28\u65E5", "29\u65E5", "30\u65E5"];
  constructor(year, month, day) {
    const y = HijriDay.numeric(year, "hijri year");
    const m = HijriDay.numeric(month, "hijri month");
    const d = HijriDay.numeric(day, "hijri day");
...
class Event extends AbstractCulture {
  ...
  getCharIndex(index) {
    return EventManager.CHARS.indexOf(this.data.charAt(index));
  }
  getValue(index) {
  ...
  getSolarDayBySolarDay(year) {
    ...
    const y = month[0];
    const m = month[1];
    const d = this.getValue(3);
    const delay = this.getValue(4);
    const lastDay = SolarMonth.fromYm(y, m).getDayCount();
    if (d > lastDay) {
      if (delay === 0) {
        return null;
      }
  ...
  getSolarDayByLunarDay(year) {
    ...
    const d = this.getValue(3);
    const delay = this.getValue(4);
    const lastDay = LunarMonth.fromYm(y, m).getDayCount();
    if (d > lastDay) {
      if (delay === 0) {
        return null;
      }
  ...
  getSolarDayByWeek(year) {
    const n = this.getValue(3);
    if (n === 0) {
      return null;
    }
    const m = SolarMonth.fromYm(year, this.getValue(2));
    const w = this.getValue(4);
    if (n > 0) {
  ...
  getSolarDayByTerm(year) {
    const d = SolarTerm.fromIndex(year, this.getValue(2)).getSolarDay();
    const offset = this.getValue(4);
    return offset === 0 ? d : d.next(offset);
  }
...
class EventBuilder {
  ...
  content(t, a, b, c) {
    this.data[1] = this.encodeType(t);
    return this.setValue(2, a).setValue(3, b).setValue(4, c);
  }
  solarDay(solarMonth, solarDay, delayDays) {
    return this.content(0 /* SOLAR_DAY */, solarMonth, solarDay, delayDays);
  }
  lunarDay(lunarMonth, lunarDay, delayDays) {
    return this.content(2 /* LUNAR_DAY */, lunarMonth, lunarDay, delayDays);
  }
  solarWeek(solarMonth, weekIndex, week) {
    return this.content(1 /* SOLAR_WEEK */, solarMonth, weekIndex, week);
  }
  termDay(termIndex, delayDays) {
    return this.content(3 /* TERM_DAY */, termIndex, 0, delayDays);
  }
  termHeavenStem(termIndex, heavenStemIndex, delayDays) {
    return this.content(4 /* TERM_HS */, termIndex, heavenStemIndex, delayDays);
  }
  termEarthBranch(termIndex, earthBranchIndex, delayDays) {
    return this.content(5 /* TERM_EB */, termIndex, earthBranchIndex, delayDays);
  }
  startYear(year) {
    const size = EventManager.CHARS.length;
    let n = year;
...
class EventManager {
  ...
  static saveOrUpdate(name, data) {
    const o = EventManager.REGEX + name;
    const matcher = new RegExp(o, "g").exec(EventManager.DATA);
    if (matcher) {
